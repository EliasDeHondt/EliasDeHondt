<h1 align="center">Hi , I'm Elias De Hondt <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28"></h1>

<p align="center">
  <img src="https://github.com/user-attachments/assets/cdf2c925-42b4-4b14-8429-3d5b02abe639" style="width: 1000px; height: auto;" />
</p>

## 🙋‍♂️ About Me
  - ⚡ About Myself : 
      - **Keen observer. ✨**
      - **Always appreciate art. 🎭**
      - **Open to read and learn anything I'm interested in. 😅**
      - **Like to read Books, watch Documentary, watch Movies. 😉**
   
## 🏆 Personal & Business Values
  - **Reliability:** Focus on security and availability, essential for my web hosting company.
  - **Customer-Centric:** Building relationships by providing tailored solutions that directly address customer needs.

## 🖥️ Tools & Technologies
  - **Development & Management:** Git, Linux CLI, VS Code
  - **Cloud Services:** Google Cloud, Docker, Kubernetes
  - **Frameworks & Languages:** .NET, Python, SQL, Bash
